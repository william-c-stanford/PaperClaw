...
Ключи хранятся только на стороне сервера в `saves/settings.json` (режим `600`) и никогда не
...
**переменные окружения → `.env` (cwd) → `.env` в `$PAPERCLAW_HOME` → `settings.json`**.
...
Ключи хранятся на стороне сервера в `saves/settings.json` (режим `600`) и никогда не отправляются в
