...
키는 서버 측 `saves/settings.json`(모드 `600`)에만 저장되며 브라우저로 절대 전송되지 않습니다.
...
**환경 변수 → `.env`(현재 디렉터리) → `$PAPERCLAW_HOME` 의 `.env` → `settings.json`**.
...
**제 API 키는 안전한가요?**
키는 서버 측 `saves/settings.json`(모드 `600`)에만 저장되며 브라우저로 전송되거나 로그에 남지 않습니다.
