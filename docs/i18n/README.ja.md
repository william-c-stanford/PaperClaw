...
キーはサーバー側の `saves/settings.json`（モード `600`）にのみ保存され、ブラウザには決して送られません。
...
**環境変数 → `.env`（カレント）→ `$PAPERCLAW_HOME` 内の `.env` → `settings.json`**。
...
**私の API キーは安全ですか？**
キーはサーバー側の `saves/settings.json`（モード `600`）にのみ保存され、ブラウザに送られたりログに記録されたりしません。
