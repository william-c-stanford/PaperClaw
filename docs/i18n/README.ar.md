...
تُخزَّن المفاتيح على الخادم فقط في `saves/settings.json` (الوضع `600`) ولا تُرسَل أبدًا إلى المتصفح.
...
**متغيّرات البيئة → `.env` (المجلد الحالي) → `.env` في `$PAPERCLAW_HOME` → `settings.json`**.
...
تُخزَّن المفاتيح على الخادم في `saves/settings.json` (الوضع `600`) ولا تُرسَل أبدًا إلى المتصفح ولا تُسجَّل.
